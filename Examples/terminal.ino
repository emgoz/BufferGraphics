...
void setup()
{
    ...
    display.setBrightness(br); //low brightness to save energy
    buff.clear();
...
void loop()
{
  if (Serial.available() > 0){
    byte c = Serial.read();
    if ( c == '+') display.setBrightness(br+=4); 
    else if ( c == '-') display.setBrightness(br-=4);
    else if (c == 13) buff.clear();
    else if (c == 127){
      for (int i = 0; i < 8; i++){
      buff.scrollRight(1);
      delay(15);
...
void printChar(byte c, int x, int y){
    for (int a=0;a<12;a++){                             
        buff.writeByte(x,y+a,pgm_read_byte_near(font + (c * 12) + a),ON);
    }
}
